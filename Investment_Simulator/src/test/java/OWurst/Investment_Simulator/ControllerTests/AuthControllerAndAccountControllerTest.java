...
class AuthControllerAndAccountControllerTest {
    ...
    // UserDTO testDTO = new UserDTO("user", "Password1$", "firstname", "lastname",
    // "email@gmail.com");
    ...
    // ResponseEntity<String> response = authController.saveUser(badUserDTO,
    // request);
    // assertEquals(HttpStatus.BAD_REQUEST, response.getStatusCode());
    ...
    // UserDTO userDTO = new UserDTO("userooni", "billiam$Y", "ridderson", "",
    // "x@x");
    ...
    // loginUser();
    // ResponseEntity<String> response =
    // accountController.updateEmail("bobby@gmail.com", request);
    // assertEquals(HttpStatus.OK, response.getStatusCode());

    // ObjectMapper mapper = new ObjectMapper();
    // UserDTO user = null;
    // try {
    // user = mapper.readValue(accountController.getUserObject(request).getBody(),
    // UserDTO.class);
    // } catch (Exception e) {
    // }
